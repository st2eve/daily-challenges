<script>
  export default{
    ...
    methods: {
      ...
      resultExo4(){
        if(this.answer4.length == 0 || this.answer4.length == 1 && this.sortArray > this.answer4[0] || this.sortArray > this.answer4[this.answer4.length-1]){
          this.answer4.push(this.sortArray);
        }else if(this.answer4.length == 1 && this.sortArray < this.answer4[0] || this.sortArray < this.answer4[0]){
          this.answer4.unshift(this.sortArray);
        }else{
          for(let i = 0; i < this.answer4.length; i++){
            if(this.answer4[i] < this.sortArray && this.sortArray < this.answer4[(i+1)]){
              this.answer4.splice(i+1, 0, this.sortArray);
              break;
            }else if(this.answer4[i] > this.sortArray && this.sortArray > this.answer4[(i-1)]){
              this.answer4.splice((i-1), 0, this.sortArray);
              break;
...
<template>
  <main class="container__main">
    <div class="main__exo1">
      <h2>
        Find the sum of all the multiples of {{ multiple1 }} or {{ multiple2 }} below 1000
      </h2>
    ...
    <div class="main__exo4">
      <h2>
        Write a function that will accept an array of integers and sort them by ascending number and eliminating duplicates.
      </h2>
      <div class="form__sortArray">
        <label>
          Add {{ sortArray }} in the Array
        </label>
        <input class="multiples" type="number" v-model="sortArray">
      </div>
      <button @click="resultExo4">
        Submit
      </button>
      <div class="form__answer">
        <p>
...
<style scoped>
  ...
  .main__exo1, .main__exo2, .main__exo3, .main__exo4{
    display: flex;
    flex-direction: column;
    gap: 20px;
  }
